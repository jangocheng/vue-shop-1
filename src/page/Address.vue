        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="{path:'orderconfirm',query:{'addressId':addrId}}">提交订单</router-link>
            </div>
          </div>
        </div>
      </div>
  ...
  export default{
      ...
      methods:{
        ...
        setDefault(item) {
          ...
          axios.get('/api/users/setDefauleAdr', {
            params:param
          }).then( (res) => {
            res = res.data;
            if(res.status === '1') {
